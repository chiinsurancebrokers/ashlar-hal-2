...
from backend.app.rates.registry import quote_current, quote_shortlist, quote_exclusions
from backend.app.services.leads import classify_journey, lead_intent, journey_cta
from backend.app.services.discovery import apply_discovery_answer, next_discovery_question, discovery_progress
from backend.app.services.openai_adviser import openai_adviser_response
from backend.app.travel.service import travel_turn
...
ALLOWED_FIELDS = {
    "first_name","age","residence_country","nationality","coverage_area","currency",
    "deductible","budget_annual","client_segment","journey",*BOOL_FIELDS,
    "pending_question","deductible_answered","deductible_preference","outpatient_answered",
    "maternity_answered","dental_answered","mental_health_answered","evacuation_answered",
    "budget_answered","budget_preference","discovery_complete",
}
...
def _clean_updates(updates: dict[str, Any]) -> dict[str, Any]:
    ...
    for key,value in (updates or {}).items():
        if key not in ALLOWED_FIELDS or value is None:
            continue
        if key in BOOL_FIELDS or key in {"deductible_answered","outpatient_answered","maternity_answered","dental_answered","mental_health_answered","evacuation_answered","budget_answered","discovery_complete"}:
            ...
            if value in AREA_HELP:
                clean[key]=value
        elif key in {"client_segment","journey","pending_question","deductible_preference","budget_preference"}:
...
def _deterministic_updates(message: str, state: dict, history: list[dict] | None = None) -> dict:
    ...
    # Simple introduction/name extraction. Keep only a short display name.
    if not state.get("first_name"):
        name_patterns=[
            r"\bmy name is\s+([A-Za-zÀ-ÖØ-öø-ÿΑ-Ωα-ωΆ-ώ'-]{2,40})",
            r"\bi(?:'m| am)\s+called\s+([A-Za-zÀ-ÖØ-öø-ÿΑ-Ωα-ωΆ-ώ'-]{2,40})",
            r"\bμε λένε\s+([A-Za-zÀ-ÖØ-öø-ÿΑ-Ωα-ωΆ-ώ'-]{2,40})",
            r"\bμε λενε\s+([A-Za-zÀ-ÖØ-öø-ÿΑ-Ωα-ωΆ-ώ'-]{2,40})",
            r"\bονομάζομαι\s+([A-Za-zÀ-ÖØ-öø-ÿΑ-Ωα-ωΆ-ώ'-]{2,40})",
            r"\bονομαζομαι\s+([A-Za-zÀ-ÖØ-öø-ÿΑ-Ωα-ωΆ-ώ'-]{2,40})",
        ]
...
def _quote_payload(state: dict) -> list[dict]:
    if not _complete_for_quote(state) or not state.get("discovery_complete"):
        return []
    if not _complete_for_quote(state):
        return []
    try:
        applicant=Applicant(**state)
        return [q.model_dump() for q in quote_shortlist(applicant,limit=5)]
    except Exception:
        return []


def _excluded_payload(state: dict) -> list[dict]:
    if not _complete_for_quote(state) or not state.get("discovery_complete"):
        return []
    if not _complete_for_quote(state):
        return []
    try:
        return quote_exclusions(Applicant(**state))
    except Exception:
        return []
...
def _intent_flags(message: str, state: dict) -> dict:
    ...
    destination_phrases=[
        "healthcare in greece","health care in greece","greek healthcare","greek health system",
        "health system in greece","healthcare system in greece","public healthcare in greece",
        "public health system in greece","public hospitals in greece","waiting lists in greece",
        "waiting times in greece","how is healthcare in greece","how is the healthcare in greece",
        "why private insurance in greece","why do i need private insurance in greece",
        "σύστημα υγείας στην ελλάδα","συστημα υγειας στην ελλαδα",
        "δημόσιο σύστημα υγείας","δημοσιο συστημα υγειας",
        "δημόσια νοσοκομεία στην ελλάδα","δημοσια νοσοκομεια στην ελλαδα",
        "λίστες αναμονής","λιστες αναμονης","χρόνοι αναμονής","χρονοι αναμονης",
        "γιατί ιδιωτική ασφάλιση στην ελλάδα","γιατι ιδιωτικη ασφαλιση στην ελλαδα",
    ]

    return {
        "provider": any(x in lower for x in ["morgan price","morgan","provider","company","insurer","εταιρ","πάροχ","ασφαλιστ"]),
        # Mentioning "international health insurance" states the product intent; it does
        # not by itself request an educational lecture.
        "education": any(x in lower for x in [
            "international vs local","international or local","why international","why ipmi",
            "local insurance vs international","γιατί international","γιατι international",
            "διεθνές ή τοπικό","διεθνες ή τοπικο","διεθνές vs τοπικό","διεθνες vs τοπικο"
        ]),
        # Residence/place names alone must never trigger a country healthcare profile.
        "destination": any(x in lower for x in destination_phrases),
...
async def _openai_intake_analysis(
    message: str,
    state: dict,
    history: list[dict] | None,
    greek: bool,
) -> dict:
    """Understand natural applicant language without surrendering deterministic control."""
    instructions=f"""
You are HAL's applicant-understanding layer for Ashlar Assurance.
...
- Do not calculate premiums or recommend a plan.
- Do not invent benefits, exclusions, underwriting outcomes or eligibility.
- Extract only facts the user actually stated or clearly confirmed.
- Never infer nationality from residence.
- If the user gives their name, store only first_name.
- If the user says they live/reside in a country, set residence_country to that country.
- journey can be ipmi, travel or local_review only when the user's intent is clear.
- Coverage area codes:
  area1 = Europe
  area2 = Worldwide excluding USA, Singapore, Hong Kong and China
  area3 = Worldwide excluding USA
  area4 = Worldwide including USA
- Use only these applicant_updates keys:
  first_name, age, residence_country, nationality, coverage_area, currency,
  deductible, budget_annual, journey, client_segment,
...
"""
    try:
        text=await openai_adviser_response(
        ...
        return {
            ...
            "applicant_updates": _clean_updates(parsed.get("applicant_updates",{})),
        }
    except Exception:
        return {"acknowledgement":"","applicant_updates":{}}


def _first_turn_intro(state: dict, greek: bool, history: list[dict] | None, journey: str) -> str:
    """Warm greeting for a genuine first enquiry; never repeat on later turns."""
    if history:
        return ""
    name=(state.get("first_name") or "").strip()
    salutation=(f"Γεια σου {name}, χαίρομαι πολύ για τη γνωριμία. " if greek and name
                else "Γεια σας, χαίρομαι πολύ για τη γνωριμία. " if greek
                else f"Hi {name}, nice to meet you. " if name
                else "Hi, nice to meet you. ")

    if journey=="ipmi":
        if str(state.get("residence_country","")).lower() in {"greece","gr","hellas","ελλάδα","ελλαδα"}:
            body=("Η Ελλάδα είναι μια εξαιρετική χώρα για να ζει κανείς και ένα international health insurance plan μπορεί να είναι εξαιρετική επιλογή όταν θέλετε ευρύτερη πρόσβαση σε ιδιωτικούς παρόχους, υψηλά όρια και δυνατότητα περίθαλψης και εκτός Ελλάδας. Αφού μου είπατε ότι ζείτε στην Ελλάδα, θα το αντιμετωπίσω ως μακροχρόνια ανάγκη IPMI και όχι ως travel insurance. "
                  if greek else
                  "Greece is an excellent place to live, and an international health insurance plan can be an excellent choice when you value broader private-provider access, high limits and the option of treatment beyond Greece. Since you told me you live in Greece, I’ll treat this as a long-term IPMI enquiry rather than travel insurance. ")
        else:
            body=("Θα σας καθοδηγήσω βήμα-βήμα ώστε να βρούμε το international plan που ταιριάζει πραγματικά στις ανάγκες σας. "
                  if greek else
                  "I’ll guide you step by step so we can identify the international plan that genuinely fits your needs. ")
...
async def chat_turn(message: str, state: dict, history: list[dict] | None = None) -> dict:
    ...
    state=_merge_state(state, apply_discovery_answer(message,state))

    openai_ack=""
    provisional_journey=classify_journey(message,state)
    if settings.openai_api_key and provisional_journey not in {"travel","local_review"} and not state.get("discovery_complete"):
        intake=await _openai_intake_analysis(message,state,history,greek)
        state=_merge_state(state,intake.get("applicant_updates",{}))
        openai_ack=str(intake.get("acknowledgement","") or "").strip()

    journey=classify_journey(message,state)
    state["journey"]=journey
    flags=_intent_flags(message,state)

    # Journey routing: local review / IPMI. Travel is handled above.
    if journey=="local_review":
        reply,sources=_local_review_direct_answer(greek)
        return {
            "reply":reply,"state":state,"quotes":[],"excluded_plans":[],"ai_status":"journey_router",
    ...
    if flags["destination"]:
        ...
        return {"reply":reply,"state":state,"quotes":_quote_payload(state),"excluded_plans":_excluded_payload(state),"ai_status":"curated_knowledge",
                "provider_sources_used":[],"knowledge_sources_used":sources,
    ...
    # Guided adviser flow: do not shortlist on age/residence/area alone.
    if journey not in {"travel","local_review"}:
        next_q=next_discovery_question(state,greek)
        if next_q is not None:
            state["pending_question"]=next_q["key"]
            state["discovery_complete"]=False
            intro=openai_ack or _first_turn_intro(state,greek,history,journey if journey!="undetermined" else "ipmi")
            guided_reply=((intro.rstrip()+" "+next_q["reply"]) if intro else next_q["reply"]).strip()
            return {
                "reply":guided_reply,"state":state,"quotes":[],"excluded_plans":[],
                "ai_status":"guided_discovery","provider_sources_used":[],"knowledge_sources_used":[],
                "journey":journey if journey!="undetermined" else "ipmi",
                "lead_cta":journey_cta("ipmi",False),"open_application_form":False,
                "quick_replies":next_q["quick_replies"],"discovery":discovery_progress(state),
            }
        state["pending_question"]=None
        state["discovery_complete"]=True
        # The final discovery answer should immediately produce a useful shortlist.
        if not any(x in message.lower() for x in ["tell me about","why international","healthcare system","morgan price"]):
            reply=("Τέλεια — τώρα έχω αρκετά στοιχεία για να συγκρίνω τα plans με βάση τις πραγματικές σας προτεραιότητες. Παρακάτω είναι το shortlist του HAL." if greek else "Great — I now have enough information to compare the plans around your actual priorities. Here is HAL's shortlist.")
            return {
                "reply":reply,"state":state,"quotes":_quote_payload(state),"excluded_plans":_excluded_payload(state),
                "ai_status":"guided_shortlist","provider_sources_used":[],"knowledge_sources_used":[],
                "journey":"ipmi","lead_cta":journey_cta("ipmi",True),"open_application_form":False,
                "quick_replies":[],"discovery":discovery_progress(state),
            }
    ...
    # Even without OpenAI, deterministic intake keeps moving.
    if not settings.openai_api_key:
        return {
            "reply":_fallback_reply(state,greek),
            "state":state,
            "quotes":_quote_payload(state),
            "excluded_plans":_excluded_payload(state),
...
"""
    ...
    if reply in {"","...","…",".."}:
        ...
        ai_status="openai_responses_api" if not llm_error else "openai_degraded_fallback"
