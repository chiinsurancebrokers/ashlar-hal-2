...
IPMI_VALUE_PATH = BASE / "knowledge" / "ipmi_value_proposition.json"
...
@lru_cache
def ipmi_value_proposition() -> dict:
    return json.loads(IPMI_VALUE_PATH.read_text(encoding="utf-8"))
...
def destination_context(country_code: str) -> str:
    ...
    for f in data["facts"]:
        ...
        parts.append(line)
    if data.get("insurance_argumentation"):
        ia=data["insurance_argumentation"]
        parts.append("INSURANCE ARGUMENTATION FRAMEWORK:\n" + ia["objective"])
        parts.append("BALANCED ARGUMENT STRUCTURE:\n- " + "\n- ".join(ia["balanced_structure"]))
        parts.append(
            "WAITING LISTS — HIGH PRIORITY:\n"
            + ia["waiting_lists"]["instruction"]
            + "\nBOUNDARY: " + ia["waiting_lists"]["boundary"]
        )
        for ex in ia.get("evidence_to_argument_examples", []):
            parts.append(
                f'EVIDENCE → INSURANCE ARGUMENT: {ex["evidence"]}\n'
                f'{ex["insurance_argument"]}'
            )
        parts.append("TONE:\n- " + "\n- ".join(ia["tone"]))
...
def ipmi_value_context() -> str:
    data=ipmi_value_proposition()
    parts=[
        "COMPREHENSIVE IPMI VALUE PROPOSITION",
        "DEFAULT SALES THESIS: " + data["positioning"]["default_sales_thesis"],
        "ACCURACY BOUNDARY: " + data["positioning"]["accuracy_boundary"],
    ]
    for p in data["pillars"]:
        parts.append(
            f'{p["title"]}: {p["argument"]}\n'
            f'SALES TRANSLATION: {p["sales_translation"]}\n'
            f'GUARDRAIL: {p["guardrail"]}'
        )
    parts.append("VERIFIED MORGAN PRICE 2026 EXAMPLES:\n- " + "\n- ".join(data["morgan_price_2026_verified_examples"]["examples"]))
    parts.append("PROMPT RULES:\n- " + "\n- ".join(data["prompt_rules"]))
    parts.append("EDUCATIONAL SOURCES:\n- " + "\n- ".join(f'{s["name"]}: {s["url"]}' for s in data["sources"]))
    return "\n\n".join(parts)
