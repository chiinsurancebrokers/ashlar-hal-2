...
def _history_input(history: list[dict] | None, message: str) -> list[dict[str, str]]:
    items: list[dict[str, str]] = []
    for item in (history or [])[-8:]:
        role = item.get("role")
        content = str(item.get("content") or "").strip()
        if role in {"user", "assistant"} and content and content not in {"...", "…"}:
...
def _request_body(
    *,
    ...
    body: dict[str, Any] = {
        ...
        "input": _history_input(history, message),
        "store": False,
...
async def openai_adviser_response(
    *,
    instructions: str,
    message: str,
